...
t.test('openS3Stream - reports S3 errors', async t => {
  ...
  t.equal(logger.debug.getCall(0).lastArg, 'S3 Error, URL: s3://bucket/key Error: "FAILED" attempt 1 / 3')
  t.equal(logger.debug.getCall(1).lastArg, 'S3 Error, URL: s3://bucket/key Error: "FAILED" attempt 2 / 3')
  t.equal(logger.debug.getCall(2).lastArg, 'S3 Error, URL: s3://bucket/key Error: "FAILED" attempt 3 / 3')
...
t.test('readDynamoItem - reports DynamoDB errors', async t => {
  ...
  t.equal(logger.debug.getCall(0).lastArg, 'DynamoDB Error, attempt 1 / 3')
  t.equal(logger.debug.getCall(1).lastArg, 'DynamoDB Error, attempt 2 / 3')
  t.equal(logger.debug.getCall(2).lastArg, 'DynamoDB Error, attempt 3 / 3')
...
t.test('deleteDynamoItem - reports DynamoDB errors', async t => {
  ...
  await t.rejects(() => deleteDynamoItem('TABLE', 'KEY', 'VALUE', 'car-file'), { message: 'Cannot send command to DynamoDB' })
  t.equal(logger.debug.getCall(0).lastArg, 'DynamoDB Error, attempt 1 / 3')
  t.equal(logger.debug.getCall(1).lastArg, 'DynamoDB Error, attempt 2 / 3')
  t.equal(logger.debug.getCall(2).lastArg, 'DynamoDB Error, attempt 3 / 3')
  t.equal(logger.error.getCall(0).lastArg, 'Cannot send command to DynamoDB after 3 attempts')
...
t.test('publishToSQS - reports SQS errors', async t => {
  ...
  await t.rejects(() => publishToSQS('QUEUE', 'DATA', 'car-file'), { message: 'FAILED' })
})
